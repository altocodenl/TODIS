...
An important point is that an unified dataspace doesn't require the data to be in one place. The dataspace is a way to bring together data that exists in different places. A single dataspace is unified because it unifies, not because everything has to be placed inside it before we can use it. The dataspace is where data comes together.
...
Armed with the pattern of call and response, we can now explore what happens between one and the other. Any call happening in a DIS will have a [deliberate](https://en.wikipedia.org/wiki/Teleology) response. The purpose of a call is to become a response. Logic is **how** this transformation happens. In other words, logic is intentional transformation of data. Call and response represent communication; logic represents transformation.
...
1. We could restrict the system to process transactions involving a certain account one at a time. Whether this is achieved by [queues](https://en.wikipedia.org/wiki/Queue_(abstract_data_type)), [locks](https://en.wikipedia.org/wiki/Lock_(computer_science)), [database transactions](https://en.wikipedia.org/wiki/Database_transaction) or [consensus algorithms](https://en.wikipedia.org/wiki/Consensus_(computer_science)), the upshot is the same: one call need to wait for the other. This is choosing **consistency over performance**.
